...
| Fixed: dashboard | Pinned containers, projects, and volumes; fleet volume/update totals; failed-activity badge and attention count; legacy fleet totals | Pins load complete resource collections. Fleet totals fail atomically instead of displaying partial sums. The update total uses dashboard action items (impacted standalone containers plus projects), not the raw image-update count. Failed activities use the filtered pagination total rather than the size of a bounded page. |
